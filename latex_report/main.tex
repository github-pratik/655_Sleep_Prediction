...
The system has three parts. The offline Python pipeline prepares the dataset and trains models. The VitalSync web dashboard helps visualize trends during development. The iOS application is the main mobile demo and runs the final prediction path.
...
The \textbf{VitalSync} web dashboard was used as a development and presentation aid. It shows sleep trends, HRV changes, and predicted fatigue labels in a browser. This helped us inspect the data and explain the project before relying only on the physical iPhone demo.

\subsection{iOS Native Edge Deployment}
The iOS app is written in Swift and uses HealthKit for local data access. Instead of shipping Python code or a large ML runtime, the app loads \path{mobile_linear_contract.json}. This file stores the feature order and model parameters needed for inference. The app then fetches recent HealthKit samples, computes the required features, applies the same preprocessing used during training, and returns the fatigue prediction on the phone.
...
For mobile inference, the final model path is exported as \texttt{mobile\_linear\_contract.json}. At prediction time, the iPhone app builds a feature dictionary from HealthKit-derived nightly signals. If a feature is missing, the app uses the training median stored in the contract. It then scales the values and computes the class score with vector arithmetic.

This format is simple, but that is the point for a mobile demo. The app does not need to initialize TensorFlow Lite, Core ML, or a Python runtime. In our test, the path from HealthKit data fetch through final prediction display completed in under 200 milliseconds on an iPhone 14.
...
Figure \ref{fig:demo} shows the iOS screens used to validate the system path. The screenshots cover the app interface, HealthKit-driven data loading, local prediction output, and the history view used during the demo.
...
During the demo, the user grants HealthKit access, loads Apple Watch-derived data, runs local inference from the bundled contract, and reviews the fatigue result. This confirms that the project connects the offline model pipeline to a native mobile workflow instead of remaining only an offline notebook result. All inference is performed locally, so the demo does not require a network connection or a cloud prediction service.
